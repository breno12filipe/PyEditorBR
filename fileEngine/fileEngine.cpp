...
FileEngine::FileEngine(std::string fileName) { this->fileName_ = fileName; }
...
std::string FileEngine::getFileName() { return this->fileName_; }

std::string FileEngine::getFilePath() { return this->fileName_; }
...
bool FileEngine::createFile() {
  ...
  outFile.close();
  if (!outFile) {
    std::cerr << "\033[91mERROR CREATING FILE: " << this->fileName_
              << this->fileName_ << std::endl;
  }
}

void FileEngine::saveFile(std::string &stringInput) {
    std::cout << this->fileName_;
